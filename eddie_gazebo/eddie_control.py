...
class EddieGazeboPublisher(Node):
    ...
    def __init__(self, verbose=False) -> None:
        ...
        self.arm_controller: str = (
            self.get_parameter("arm_controller").get_parameter_value().string_value
        )
        assert (
            self.arm_controller in valid_controllers["arm"]
        ), f"Undefined arm_controller: {self.arm_controller}"

        self.base_controller: str = (
            self.get_parameter("base_controller").get_parameter_value().string_value
        )
        assert (
            self.base_controller in valid_controllers["base"]
        ), f"Undefined base_controller: {self.base_controller}"

        arm_controller_name = {
            side: f"arm_{side}_joint_trajectory_controller"
            if "joint_trajectory" in self.arm_controller
            else f"arm_{side}_effort_controller"
            for side in ["left", "right"]
        }
        base_controller_name = f"base_{self.base_controller}_controller"

        self.get_logger().info(
            f"Setting up command interface for selected arm controller: {arm_controller_name}"
        )
        self.get_logger().info(
            f"Setting up command interface for selected base controller: {base_controller_name}"
        )

        with open(
            "install/eddie_gazebo/share/eddie_gazebo/config/eddie_controllers.yaml"
        ) as param_file:
        ...
        self.components = {
            "arm_left": {
                ...
                "frame_id": "base_link",
                "trajectory_duration": self.trajectory_durations[
                    self.arm_controller.split("_")[-1]
                ],
            },
            "arm_right": {
                "publisher": self.create_publisher(
        ...
        for component_name in self.components:
            if "arm" in component_name:
                if "joint_trajectory" in self.arm_controller:
                    with open(
                        "install/eddie_description/share/eddie_description/"
                        ...
                        self.components[component_name]["state"]["position"] = np.array(
                            [
                                initial_positions[f"joint_{index}"]
        ...
        if self.verbose:
            self.get_logger().info(
                "Loaded keyboard control interface with the following components:"
            )
    ...
    def evaluate_time(self, time_object: rclpy.time.Time):
        return time_object.seconds_nanoseconds()[0] + (
            time_object.seconds_nanoseconds()[1] * 1e-9
        )
    ...
    def update_state(
        ...
        if "arm" in component_name:
            if self.arm_controller.split("_")[-1] == "position":
                self.components[component_name]["state"]["position"] += increment[
                    : len(self.components[component_name]["state"]["position"])
                ]
            elif self.arm_controller.split("_")[-1] == "velocity":
                self.components[component_name]["state"]["velocity"] += increment[
                    : len(self.components[component_name]["state"]["velocity"])
                ]
            elif self.arm_controller == "effort":
                self.components[component_name]["state"]["effort"] += increment[
                    : len(self.components[component_name]["state"]["effort"])
                ]
        else:
            self.components[component_name]["state"] += increment[
                : len(self.components[component_name]["state"])
            ]
        ...
        for component_name in self.components:
            if "arm" in component_name:
                if "joint_trajectory" in self.arm_controller:
                    ...
                    trajectory_point.positions = self.components[component_name][
                        "state"
                    ]["position"].tolist()

                    trajectory_point.time_from_start = Duration(
                        sec=self.components[component_name]["trajectory_duration"][0],
                        nanosec=self.components[component_name]["trajectory_duration"][
                            1
                        ],
                    )

                    if self.arm_controller.split("_")[-1] == "velocity":
                        trajectory_point.velocities = self.components[component_name][
                            "state"
                        ]["velocity"].tolist()
                    ...
                    msg.data = self.components[component_name]["state"][
                        "effort"
                    ].tolist()
        ...
        with np.printoptions(suppress=True):
            if current_component is not None:
                ...
                if "arm" in current_component:
                    print(
                        f"Updated state of {current_component} to: ",
                        np.array(
                ...
                print(
                    "Updated states to: ",
                    [
                        np.array(self.components[component]["state"])
...
class KeyboardPress:
    def __init__(self):
        ...
        self.speed_bindings = {"+": (1.05, 1.1), "-": (0.95, 0.9)}
