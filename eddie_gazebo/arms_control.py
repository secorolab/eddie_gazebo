...
class ArmControllerPublisher(Node):
    ...
    def __init__(self):
        ...
        self.arm_positions = []
        self.arm_positions.append(
            [-0.70, -2.05, 2.14, -2.44, -1.57, 0.00, 0.00]
        )  # Left initial pos
        self.arm_positions.append(
            [0.70, -2.05, 0.90, 2.44, 1.57, 0.00, 0.00]
        )  # Right intial pos
    ...
    def timer_callback(self):
        ...
        point_arm_left.positions = (
            self.arm_positions[0]
            if (self.index % 2 == 0)
            else [0.3 * i for i in range(1, 8)]
        )
        point_arm_right.positions = (
            self.arm_positions[1]
            if (self.index % 2 == 1)
            else [0.3 * i for i in range(1, 8)]
        )

        point_arm_left.time_from_start = Duration(
            sec=int(self.duration_sec), nanosec=int(self.duration_nanosec)
        )  # Time to next position
        point_arm_right.time_from_start = Duration(
            sec=int(self.duration_sec), nanosec=int(self.duration_nanosec)
        )  # Time to next position
